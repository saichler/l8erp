...
echo "Building saichler/erp-log-agent:${TAG} for ${PLATFORM}"
docker buildx build --no-cache --platform="$PLATFORM" -t saichler/erp-log-agent:${TAG} .
docker push saichler/erp-log-agent:${TAG}
