...
func testServiceHandlersSCM(t *testing.T, vnic ifs.IVNic) {
	...
	if h, ok := bins.Bins(vnic); !ok || h == nil {
		log.Fail(t, "Bin service handler not found")
	}
	...
	if h, ok := deliveryproofs.DeliveryProofs(vnic); !ok || h == nil {
		log.Fail(t, "DeliveryProof service handler not found")
	}
	...
	if h, ok := forecastaccuracies.ForecastAccuracies(vnic); !ok || h == nil {
		log.Fail(t, "ForecastAccuracy service handler not found")
	}
	if h, ok := forecastmodels.ForecastModels(vnic); !ok || h == nil {
		log.Fail(t, "ForecastModel service handler not found")
	}
	if h, ok := freightaudits.FreightAudits(vnic); !ok || h == nil {
		log.Fail(t, "FreightAudit service handler not found")
	}
	if h, ok := freightrates.FreightRates(vnic); !ok || h == nil {
		log.Fail(t, "FreightRate service handler not found")
	}
	if h, ok := inventoryvaluations.InventoryValuations(vnic); !ok || h == nil {
		log.Fail(t, "InventoryValuation service handler not found")
	}
	...
	if h, ok := lotnumbers.LotNumbers(vnic); !ok || h == nil {
		log.Fail(t, "LotNumber service handler not found")
	}
	...
	if h, ok := packtasks.PackTasks(vnic); !ok || h == nil {
		log.Fail(t, "PackTask service handler not found")
	}
	if h, ok := picktasks.PickTasks(vnic); !ok || h == nil {
		log.Fail(t, "PickTask service handler not found")
	}
	if h, ok := polines.Polines(vnic); !ok || h == nil {
		log.Fail(t, "PurchaseOrderLine service handler not found")
	}
	...
	if h, ok := putawaytasks.PutawayTasks(vnic); !ok || h == nil {
		log.Fail(t, "PutawayTask service handler not found")
	}
	if h, ok := receivingorders.ReceivingOrders(vnic); !ok || h == nil {
		log.Fail(t, "ReceivingOrder service handler not found")
	}
	if h, ok := reorderpoints.ReorderPoints(vnic); !ok || h == nil {
		log.Fail(t, "ReorderPoint service handler not found")
	}
	if h, ok := requisitionlines.RequisitionLines(vnic); !ok || h == nil {
		log.Fail(t, "RequisitionLine service handler not found")
	}
	...
	if h, ok := serialnumbers.SerialNumbers(vnic); !ok || h == nil {
		log.Fail(t, "SerialNumber service handler not found")
	}
	if h, ok := shipments.Shipments(vnic); !ok || h == nil {
		log.Fail(t, "Shipment service handler not found")
	}
	if h, ok := shiptasks.ShipTasks(vnic); !ok || h == nil {
		log.Fail(t, "ShipTask service handler not found")
	}
	if h, ok := stockmovements.StockMovements(vnic); !ok || h == nil {
		log.Fail(t, "StockMovement service handler not found")
	}
