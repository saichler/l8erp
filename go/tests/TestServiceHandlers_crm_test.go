...
func testServiceHandlersCRM(t *testing.T, vnic ifs.IVNic) {
	...
	if h, ok := accountplans.CrmAccountPlans(vnic); !ok || h == nil {
		log.Fail(t, "CrmAccountPlan service handler not found")
	}
	if h, ok := accounts.CrmAccounts(vnic); !ok || h == nil {
		log.Fail(t, "CrmAccount service handler not found")
	}
	if h, ok := campaignmembers.CrmCampaignMembers(vnic); !ok || h == nil {
		log.Fail(t, "CrmCampaignMember service handler not found")
	}
	if h, ok := campaignresponses.CrmCampaignResponses(vnic); !ok || h == nil {
		log.Fail(t, "CrmCampaignResponse service handler not found")
	}
	if h, ok := campaignrois.CrmCampaignROIs(vnic); !ok || h == nil {
		log.Fail(t, "CrmCampaignROI service handler not found")
	}
	if h, ok := campaigns.CrmCampaigns(vnic); !ok || h == nil {
		log.Fail(t, "CrmCampaign service handler not found")
	}
	if h, ok := casecomments.CrmCaseComments(vnic); !ok || h == nil {
		log.Fail(t, "CrmCaseComment service handler not found")
	}
	...
	if h, ok := healthscores.CrmHealthScores(vnic); !ok || h == nil {
		log.Fail(t, "CrmHealthScore service handler not found")
	}
	...
	if h, ok := leadactivities.CrmLeadActivities(vnic); !ok || h == nil {
		log.Fail(t, "CrmLeadActivity service handler not found")
	}
	if h, ok := leadassigns.CrmLeadAssigns(vnic); !ok || h == nil {
		log.Fail(t, "CrmLeadAssign service handler not found")
	}
	if h, ok := leadconversions.CrmLeadConversions(vnic); !ok || h == nil {
		log.Fail(t, "CrmLeadConversion service handler not found")
	}
	...
	if h, ok := oppactivities.CrmOppActivities(vnic); !ok || h == nil {
		log.Fail(t, "CrmOppActivity service handler not found")
	}
	if h, ok := oppcompetitors.CrmOppCompetitors(vnic); !ok || h == nil {
		log.Fail(t, "CrmOppCompetitor service handler not found")
	}
	if h, ok := opportunities.CrmOpportunities(vnic); !ok || h == nil {
		log.Fail(t, "CrmOpportunity service handler not found")
	}
	if h, ok := oppproducts.CrmOppProducts(vnic); !ok || h == nil {
		log.Fail(t, "CrmOppProduct service handler not found")
	}
	if h, ok := oppstages.CrmOppStages(vnic); !ok || h == nil {
		log.Fail(t, "CrmOppStage service handler not found")
	}
	if h, ok := oppteams.CrmOppTeams(vnic); !ok || h == nil {
		log.Fail(t, "CrmOppTeam service handler not found")
	}
	...
	if h, ok := serviceparts.CrmServiceParts(vnic); !ok || h == nil {
		log.Fail(t, "CrmServicePart service handler not found")
	}
	if h, ok := serviceschedules.CrmServiceSchedules(vnic); !ok || h == nil {
		log.Fail(t, "CrmServiceSchedule service handler not found")
	}
	if h, ok := servicevisits.CrmServiceVisits(vnic); !ok || h == nil {
		log.Fail(t, "CrmServiceVisit service handler not found")
	}
