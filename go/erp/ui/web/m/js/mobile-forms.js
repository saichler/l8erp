...
(function() {
    ...
    window.MobileForms = {
         ...
         * Delegates to MobileFormFields for actual rendering
         */
        renderField(fieldConfig, value, readonly = false) {
            const type = fieldConfig.type || 'text';
            const F = MobileFormFields;
        ...
        renderForm(formDef, data = {}, readonly = false) {
            ...
            if (formDef.sections) {
                formDef.sections.forEach(section => {
                    ...
                    if (section.title) {
                        html += `<h3 class="mobile-form-section-title">${MobileUtils.escapeHtml(section.title)}</h3>`;
                    }
            ...
            form.querySelectorAll('input, select, textarea').forEach(input => {
                ...
                if (input.type === 'checkbox') {
                    ...
                    if (dateVal === 'current' || dateVal === 'n/a' || input.dataset.isZero === 'true') {
                        ...
                        formData[input.name] = MobileUtils.parseDateToTimestamp(input.value) || 0;
                    }
        ...
        showErrors(container, errors) {
            ...
            const errorHtml = `
                <div class="mobile-form-errors">
                    ${errors.map(e => `<div class="mobile-form-error">${MobileUtils.escapeHtml(e)}</div>`).join('')}
                </div>
            `;
        ...
        initReferencePickers(container) {
            ...
            refInputs.forEach(async input => {
                ...
                if (!config.endpoint) {
                    config.endpoint = MobileReferencePicker.getEndpointForModel(config.modelName);
                    if (!config.endpoint) {
                ...
                if (lookupModel && window.MobileReferenceRegistry) {
                    const registryConfig = MobileReferenceRegistry.get(lookupModel);
                    if (registryConfig) {
                ...
                const refId = input.dataset.refId;
                if (refId && refId !== '' && refId !== 'undefined') {
                    const result = await MobileReferencePicker.fetchDisplayValue(config, refId);
                    if (result) {
