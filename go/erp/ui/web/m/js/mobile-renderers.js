...
*/
/**
 * MobileRenderers - Shared rendering utilities for mobile HCM
 * Desktop Equivalent: shared/erp-renderers.js
 * REUSES MobileUtils formatting functions
 */
(function() {
    'use strict';

    window.MobileRenderers = {
        /**
        ...
        createStatusRenderer(enumMap, classMap) {
            return function(value) {
                ...
                return `<span class="status-badge ${cssClass}">${MobileUtils.escapeHtml(label)}</span>`;
            };
        },
        ...
        /**
         * Render date - delegates to MobileUtils
         */
        renderDate(timestamp) {
            return MobileUtils.formatDate(timestamp);
        },

        /**
         * Render datetime - delegates to MobileUtils
         */
        renderDateTime(timestamp) {
            return MobileUtils.formatDateTime(timestamp);
        },

        /**
         * Render money - delegates to MobileUtils
         */
        renderMoney(cents, currency) {
            return MobileUtils.formatMoney(cents, currency);
        },

        /**
         * Render percentage - delegates to MobileUtils
         */
        renderPercentage(decimal) {
            return MobileUtils.formatPercentage(decimal);
        },

        /**
         * Render phone - delegates to MobileUtils
         */
        renderPhone(phone) {
            return MobileUtils.formatPhone(phone);
        },

        /**
         * Render SSN (masked) - delegates to MobileUtils
         */
        renderSSN(ssn) {
            return MobileUtils.formatSSN(ssn);
        },
        ...
        renderPeriod(period) {
            if (!period) return '-';
            const start = period.startDate ? MobileUtils.formatDate(period.startDate) : '?';
            const end = period.endDate ? MobileUtils.formatDate(period.endDate) : '?';
            return `${start} - ${end}`;
        },
        ...
        renderEmployeeName(item) {
            ...
            return MobileUtils.escapeHtml(name) || '-';
        },
