...
*/
/**
 * Mobile PRJ Module Registry
 * Unified access to all PRJ sub-module configurations
 * Desktop Equivalent: prj/prj-config.js
 */
(function() {
    'use strict';
     ...
     * @param {string} modelName - The model name (e.g., 'PrjProject', 'PrjTask')
     ...
     * @param {string} modelName - The model name
     * @returns {string|null} Module name (e.g., 'Planning', 'Resources')
     */
    function getModuleName(modelName) {
        const mod = findModule(modelName);
        if (!mod) return null;

        if (mod === MobilePrjPlanning) return 'Planning';
        if (mod === MobilePrjResources) return 'Resources';
        if (mod === MobilePrjBilling) return 'Billing';
        if (mod === MobilePrjTimeExpense) return 'Time & Expense';
        if (mod === MobilePrjAnalytics) return 'Analytics';
