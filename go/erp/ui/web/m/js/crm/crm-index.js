...
*/
/**
 * Mobile CRM Module Registry
 * Unified access to all CRM sub-module configurations
 * Desktop Equivalent: crm/crm-config.js
 */
(function() {
    'use strict';
     ...
     * @param {string} modelName - The model name (e.g., 'CrmLead', 'CrmOpportunity')
     ...
     * @param {string} modelName - The model name
     * @returns {string|null} Module name (e.g., 'Leads', 'Opportunities')
     */
    function getModuleName(modelName) {
        const mod = findModule(modelName);
        if (!mod) return null;

        if (mod === MobileCrmLeads) return 'Leads';
        if (mod === MobileCrmOpportunities) return 'Opportunities';
        if (mod === MobileCrmAccounts) return 'Accounts';
        if (mod === MobileCrmMarketing) return 'Marketing';
        if (mod === MobileCrmService) return 'Service';
        if (mod === MobileCrmFieldService) return 'Field Service';
