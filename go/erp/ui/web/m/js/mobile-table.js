    ...
    class MobileTable {
        constructor(containerId, config) {
            ...
            this.config = {
                ...
                getStatusClass: config.getStatusClass || MobileUtils.getStatusClass,
        ...
        init() {
            if (this.config.serverSide) {
                this.debouncedFilterHandler = MobileUtils.debounce(() => {
                    this.currentPage = 1;
                    this.fetchData(1);
        ...
        _renderFilters() {
            const opts = this.config.columns.map(col => `<option value="${col.key}" ${this.filterColumn === col.key ? 'selected' : ''}>${MobileUtils.escapeHtml(col.label)}</option>`).join('');
            return `<div class="mobile-table-filters"><div class="mobile-table-filter-row"><select class="mobile-table-column-select"><option value="">All Columns</option>${opts}</select><input type="text" class="mobile-table-search-input ${this.isInvalidFilter ? 'invalid' : ''}" placeholder="${MobileUtils.escapeHtml(this.config.searchPlaceholder)}" value="${MobileUtils.escapeHtml(this.filterValue)}"></div></div>`;
        }
        ...
        _renderDefaultCard(item, index) {
            ...
            // Use col.render (matches desktop L8Table) - render may return HTML, plain values get escaped
            const getValue = (col) => {
                if (col.render) return col.render(item, index);  // render output may contain HTML
                let v = MobileUtils.getNestedValue(item, col.key);
                if (col.formatter) v = col.formatter(v, item);
                return MobileUtils.escapeHtml(v);  // escape only plain values
            };
            ...
            if (this.config.statusField) {
                const sv = MobileUtils.getNestedValue(item, this.config.statusField);
                html += `<span class="status-badge status-${this.config.getStatusClass(sv)}">${MobileUtils.escapeHtml(MobileUtils.formatStatus(sv))}</span>`;
            }
            html += '</div>';
            if (bodyCols.length > 0) {
                html += '<div class="mobile-table-card-body">';
                bodyCols.forEach(col => { html += `<div class="mobile-table-card-row"><span class="mobile-table-card-label">${MobileUtils.escapeHtml(col.label)}</span><span class="mobile-table-card-value">${getValue(col)}</span></div>`; });
                html += '</div>';
            }
            return html + '</div>';
        }
        ...
        _renderLoading() {
            return `<div class="mobile-table-loading"><div class="mobile-table-loading-spinner"></div><span>${MobileUtils.escapeHtml(this.config.loadingMessage)}</span></div>`;
        }

        _renderEmpty() {
            return `<div class="mobile-table-empty"><span class="mobile-table-empty-icon">${this.config.emptyIcon}</span><h4>No Results</h4><p>${MobileUtils.escapeHtml(this.config.emptyMessage)}</p></div>`;
        }

        _renderError() {
            return `<div class="mobile-table-error"><span>&#x26A0;</span><h4>Error Loading Data</h4><p>${MobileUtils.escapeHtml(this.errorMessage)}</p><button class="mobile-table-error-retry">Retry</button></div>`;
        }
        ...
        _applyClientFilters() {
            this.filteredData = this.config.data.filter(item => {
                ...
                if (this.filterColumn) {
                    const value = MobileUtils.getNestedValue(item, this.filterColumn);
                    return String(value).toLowerCase().includes(this.filterValue.toLowerCase());
                }
                return this.config.columns.some(col => {
                    const value = MobileUtils.getNestedValue(item, col.key);
                    return String(value).toLowerCase().includes(this.filterValue.toLowerCase());
                });
            });
            this.currentPage = 1;
        ...
        _applyClientSort() {
            ...
            this.filteredData.sort((a, b) => {
                let aVal = MobileUtils.getNestedValue(a, sortKey);
                let bVal = MobileUtils.getNestedValue(b, sortKey);
