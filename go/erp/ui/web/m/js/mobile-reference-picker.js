/**
 * MobileReferencePicker - Lookup field picker for mobile
 * Desktop Equivalent: reference_picker/reference-picker-*.js
 * Uses L8Query format exactly as desktop ERPReferencePicker.
 */
(function() {
    'use strict';

    class MobileReferencePicker {
        constructor(options = {}) {
            this.options = {
                ...
                <div class="mobile-reference-picker">
                    <div class="reference-search">
                        <input type="text" class="reference-search-input"
                               placeholder="${MobileUtils.escapeAttr(this.options.placeholder)}"
                    ...
                    <div class="reference-sort-header">
                        <button type="button" class="reference-sort-btn">
                            <span class="sort-label">${MobileUtils.escapeHtml(label)}</span>
        ...
        async loadData() {
            ...
            try {
                ...
                const response = await MobileAuth.get(`${this.options.endpoint}?body=${body}`);
                if (response) {
                ...
                MobileUtils.showError('Failed to load data');
                this.data = [];
                this.totalItems = 0;
                this.renderList();
            } finally {
        ...
        renderList() {
            ...
            if (this.data.length === 0) {
                list.innerHTML = `<div class="reference-empty">
                    <p>${MobileUtils.escapeHtml(this.options.emptyMessage)}</p>
            ...
            const html = this.data.map((item, index) => {
                ...
                return `<div class="reference-item ${isSelected ? 'selected' : ''}"
                         data-id="${MobileUtils.escapeAttr(String(id))}"
                         data-display="${MobileUtils.escapeAttr(String(display))}"
                         data-index="${index}" tabindex="0">
                        <span class="reference-radio">${isSelected ? '&#9679;' : '&#9675;'}</span>
                        <span class="reference-label">${MobileUtils.escapeHtml(String(display))}</span>
                    </div>`;
            }).join('');
            list.innerHTML = html;
        ...
        // Static Methods
        static show(options) {
            const picker = new MobileReferencePicker(options);
            picker.show();
            return picker;
        }

        static getModelConfig(modelName) {
            if (window.MobileReferenceRegistry && MobileReferenceRegistry.has(modelName)) {
                const config = MobileReferenceRegistry.get(modelName);
                return {
        ...
        static getEndpointForModel(modelName) {
            if (typeof MOBILE_NAV_CONFIG === 'undefined' || !MOBILE_NAV_CONFIG.hcm) return null;
            const services = MOBILE_NAV_CONFIG.hcm.services;
        ...
        static async fetchDisplayValue(config, idValue) {
            ...
            try {
                ...
                const response = await MobileAuth.get(`${config.endpoint}?body=${body}`);
    ...
    window.MobileReferencePicker = MobileReferencePicker;
})();
