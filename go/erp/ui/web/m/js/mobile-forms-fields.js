...
(function() {
    ...
    window.MobileFormFields = {
        renderTextField(config, value, readonly) {
            ...
            return `
                <div class="mobile-form-field">
                    <label class="mobile-form-label">${MobileUtils.escapeHtml(config.label)}${config.required ? ' *' : ''}</label>
                    <input type="${inputType}"
                           name="${config.key}"
                           value="${MobileUtils.escapeAttr(value || '')}"
                           placeholder="${MobileUtils.escapeAttr(placeholder)}"
            ...
            return `
                <div class="mobile-form-field">
                    ...
                    <textarea name="${config.key}"
                              ...
                              rows="${rows}"
                              ${requiredAttr}
                              ${readonlyAttr}>${MobileUtils.escapeHtml(value || '')}</textarea>
                </div>
            `;
        },
                ...
                return `
                    <div class="mobile-form-field">
                        ...
                        <input type="${readonly ? 'text' : 'date'}"
                               name="${config.key}"
                               value="${MobileUtils.escapeAttr(displayValue)}"
                               data-zero-label="${MobileUtils.escapeAttr(zeroLabel)}"
                               ${isZeroAttr}
                               class="mobile-form-input"
                               ${requiredAttr}
                ...
                <div class="mobile-form-field">
                    ...
                    <select name="${config.key}"
                        ...
                        ${Object.entries(options).map(([optValue, optLabel]) => `
                            <option value="${MobileUtils.escapeAttr(optValue)}" ${String(value) === String(optValue) ? 'selected' : ''}>
                                ${MobileUtils.escapeHtml(optLabel)}
                            </option>
                        `).join('')}
                    </select>
                </div>
                ...
                <div class="mobile-form-field mobile-form-field-checkbox">
                    <label class="mobile-form-checkbox-label">
                        <input type="checkbox"
                               ...
                               ${checkedAttr}
                               ${disabledAttr}>
                        <span class="mobile-form-checkbox-text">${MobileUtils.escapeHtml(config.label)}</span>
                    </label>
                </div>
            `;
        },
        ...
        renderSSNField(config, value, readonly) {
            const requiredAttr = config.required ? 'required' : '';
            const readonlyAttr = readonly ? 'readonly' : '';
            const displayValue = readonly && value ? MobileUtils.formatSSN(value) : (value || '');
            ...
            return `
                <div class="mobile-form-field">
                    ...
                    <div class="mobile-form-input-group">
                        <input type="color"
                               name="${config.key}"
                               value="${MobileUtils.escapeAttr(colorValue)}"
        ...
        renderReferenceField(config, value, readonly) {
            ...
            // Build reference config from MobileReferenceRegistry
            let refConfig = config.referenceConfig || {};
            if (config.lookupModel && window.MobileReferenceRegistry) {
                const registryConfig = MobileReferenceRegistry.get(config.lookupModel);
                if (registryConfig) {
                    refConfig = {
            ...
            if (readonly) {
                return `
                    <div class="mobile-form-field">
                        <label class="mobile-form-label">${MobileUtils.escapeHtml(config.label)}</label>
                        <input type="text"
                               name="${config.key}"
                               value="${MobileUtils.escapeAttr(displayValue || 'N/A')}"
                               data-ref-id="${MobileUtils.escapeAttr(String(refId))}"
            ...
            return `
                <div class="mobile-form-field">
                    ...
                    <input type="text"
                           ...
                           value="${MobileUtils.escapeAttr(displayValue)}"
                           data-ref-id="${MobileUtils.escapeAttr(String(refId))}"
                           data-ref-config='${MobileUtils.escapeAttr(JSON.stringify(serializableConfig))}'
                           data-field-key="${MobileUtils.escapeAttr(config.key)}"
                           data-lookup-model="${MobileUtils.escapeAttr(config.lookupModel || refConfig.modelName || '')}"
                           class="mobile-form-input reference-input"
                           ${requiredAttr}
                           readonly
