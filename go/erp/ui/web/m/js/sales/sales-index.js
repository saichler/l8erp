...
*/
/**
 * Mobile Sales Module Registry
 * Unified access to all Sales sub-module configurations
 * Desktop Equivalent: sales/sales-config.js
 */
(function() {
    'use strict';
     ...
     * @param {string} modelName - The model name (e.g., 'SalesOrder', 'CustomerContract')
     ...
     * @param {string} modelName - The model name
     * @returns {string|null} Module name (e.g., 'Customers', 'Orders')
     */
    function getModuleName(modelName) {
        const mod = findModule(modelName);
        if (!mod) return null;

        if (mod === MobileSalesCustomers) return 'Customers';
        if (mod === MobileSalesOrders) return 'Orders';
        if (mod === MobileSalesPricing) return 'Pricing';
        if (mod === MobileSalesShipping) return 'Shipping';
        if (mod === MobileSalesBilling) return 'Billing';
        if (mod === MobileSalesAnalytics) return 'Analytics';
    ...
    window.MobileSales = {
        ...
        modules: {
            Customers: MobileSalesCustomers,
            Orders: MobileSalesOrders,
            Pricing: MobileSalesPricing,
            Shipping: MobileSalesShipping,
            Billing: MobileSalesBilling,
            Analytics: MobileSalesAnalytics
        }
    };
