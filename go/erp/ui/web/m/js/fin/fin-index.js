...
*/
/**
 * Mobile FIN Module Registry
 * Unified access to all FIN sub-module configurations
 * Desktop Equivalent: fin/fin-config.js
 */
(function() {
    'use strict';
    ...
    const modules = [
        MobileGeneralLedger,
        MobileAccountsPayable,
        MobileAccountsReceivable,
        MobileCashManagement,
        MobileFixedAssets,
        MobileBudgeting,
        MobileTaxManagement
    ];
     ...
     * @param {string} modelName - The model name (e.g., 'Account', 'Vendor')
     ...
     * @param {string} modelName - The model name
     * @returns {string|null} Module name (e.g., 'General Ledger', 'Accounts Payable')
     */
    function getModuleName(modelName) {
        const mod = findModule(modelName);
        if (!mod) return null;

        if (mod === MobileGeneralLedger) return 'General Ledger';
        if (mod === MobileAccountsPayable) return 'Accounts Payable';
        if (mod === MobileAccountsReceivable) return 'Accounts Receivable';
        if (mod === MobileCashManagement) return 'Cash Management';
        if (mod === MobileFixedAssets) return 'Fixed Assets';
        if (mod === MobileBudgeting) return 'Budgeting';
        if (mod === MobileTaxManagement) return 'Tax Management';
    ...
    window.MobileFIN = {
        ...
        modules: {
            GeneralLedger: MobileGeneralLedger,
            AccountsPayable: MobileAccountsPayable,
            AccountsReceivable: MobileAccountsReceivable,
            CashManagement: MobileCashManagement,
            FixedAssets: MobileFixedAssets,
            Budgeting: MobileBudgeting,
            TaxManagement: MobileTaxManagement
        }
    };
