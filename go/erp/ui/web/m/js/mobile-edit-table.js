/**
 * MobileEditTable - Card-based data display with CRUD operations
 * Desktop Equivalent: edit_table/table.js (L8Table)
 */
(function() {
    'use strict';
    ...
    class MobileEditTable extends MobileTable {
        ...
        _defaultGetItemId(item) {
            const refConfig = MobileConfig.getReferenceConfig(this.config.modelName);
        ...
        _renderPagination() {
            ...
            if (this.config.onAdd) {
                html += `<button class="mobile-edit-table-add-btn" data-action="add">+ ${MobileUtils.escapeHtml(this.config.addButtonText)}</button>`;
            }
        ...
        _renderDefaultCard(item, index) {
            ...
            // Get value using col.render (matches desktop L8Table) - render may return HTML
            const getValue = (col) => {
                ...
                return MobileUtils.escapeHtml(v);  // escape only plain values, not render output
            };

            let html = `<div class="mobile-edit-table-card" data-index="${index}" data-id="${MobileUtils.escapeAttr(itemId)}">`;
            ...
            if (this.config.statusField) {
                const statusValue = MobileUtils.getNestedValue(item, this.config.statusField);
                const statusClass = this.config.getStatusClass(statusValue);
                html += `<span class="status-badge status-${statusClass}">${MobileUtils.escapeHtml(MobileUtils.formatStatus(statusValue))}</span>`;
            }
                ...
                bodyCols.forEach(col => {
                    html += `
                        <div class="mobile-edit-table-card-row">
                            <span class="mobile-edit-table-card-label">${MobileUtils.escapeHtml(col.label)}</span>
        ...
        _renderCardActions(item, itemId) {
            ...
            if (this.config.onEdit) {
                html += `<button class="mobile-edit-table-action-btn edit" data-action="edit" data-id="${MobileUtils.escapeAttr(itemId)}">Edit</button>`;
            }

            if (this.config.onDelete) {
                html += `<button class="mobile-edit-table-action-btn delete" data-action="delete" data-id="${MobileUtils.escapeAttr(itemId)}">Delete</button>`;
            }
        ...
        _renderEmpty() {
            ...
            if (this.config.onAdd) {
                html += `<button class="mobile-edit-table-add-btn large" data-action="add">+ ${MobileUtils.escapeHtml(this.config.addButtonText)}</button>`;
            }

            html += `
                <span class="mobile-edit-table-empty-icon">${this.config.emptyIcon}</span>
                <h4>No Results</h4>
                <p>${MobileUtils.escapeHtml(this.config.emptyMessage)}</p>
            </div>`;
        ...
        _renderLoading() {
            return `<div class="mobile-edit-table-loading"><div class="mobile-edit-table-loading-spinner"></div><span>${MobileUtils.escapeHtml(this.config.loadingMessage)}</span></div>`;
        }

        _renderError() {
            return `<div class="mobile-edit-table-error"><span>&#x26A0;</span><h4>Error Loading Data</h4><p>${MobileUtils.escapeHtml(this.errorMessage)}</p><button class="mobile-edit-table-error-retry">Retry</button></div>`;
        }
