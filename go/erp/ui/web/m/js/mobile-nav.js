/**
 * MobileNav - Hierarchical Navigation System
...
(function() {
    ...
    window.MobileNav = {
        ...
        showComingSoon(key) {
            ...
            content.innerHTML = `
                <div class="nav-header">
                    ...
                    <div class="nav-title">
                        <h1>${MobileUtils.escapeHtml(key)}</h1>
                    </div>
                </div>
        ...
        async _showRecordDetails(serviceConfig, formDef, item) {
            ...
            if (!freshRecord) {
                MobileUtils.showError('Record not found');
                return;
            }

            // Generate SAME form as edit (no readonly flag) - matches desktop hcm-crud.js:105
            const content = MobileForms.renderForm(formDef, freshRecord);
            ...
            MobilePopup.show({
                ...
                onShow: (popup) => {
                    ...
                    MobileForms.initFormFields(popup.body);
        ...
        async _openServiceForm(serviceConfig, formDef, item) {
            ...
            const content = MobileForms.renderForm(formDef, recordData);
            ...
            MobilePopup.show({
                ...
                onSave: async (popup) => {
                    ...
                    try {
                        ...
                        MobileUtils.showError(`Failed to save ${serviceConfig.label.replace(/s$/, '').toLowerCase()}`);
                    }
                }
            });
        },
        ...
        async _deleteServiceRecord(serviceConfig, id, item) {
            ...
            if (confirmed) {
                try {
                    ...
                    MobileUtils.showError(`Failed to delete ${serviceConfig.label.replace(/s$/, '').toLowerCase()}`);
                }
            }
        },
        ...
        _getServiceColumns(serviceConfig) {
            ...
            return [
                ...
                { key: 'status', label: 'Status', sortKey: 'status', render: (item) => MobileRenderers.renderBoolean(item.status) }
            ];
        },
