...
*/
/**
 * Mobile Manufacturing Module Registry
 * Unified access to all Manufacturing sub-module configurations
 * Desktop Equivalent: mfg/mfg-config.js
 */
(function() {
    'use strict';
     ...
     * @param {string} modelName - The model name (e.g., 'MfgWorkOrder', 'MfgBOM')
     ...
     * @param {string} modelName - The model name
     * @returns {string|null} Module name (e.g., 'Engineering', 'Production')
     */
    function getModuleName(modelName) {
        const mod = findModule(modelName);
        if (!mod) return null;

        if (mod === MobileMfgEngineering) return 'Engineering';
        if (mod === MobileMfgProduction) return 'Production';
        if (mod === MobileMfgShopFloor) return 'Shop Floor';
        if (mod === MobileMfgQuality) return 'Quality';
        if (mod === MobileMfgPlanning) return 'Planning';
        if (mod === MobileMfgCosting) return 'Costing';
