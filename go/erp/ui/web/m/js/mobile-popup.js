...
*/
/**
 * MobilePopup - Full-screen modal component for ERP mobile
...
(function() {
    ...
    window.MobilePopup = {
        ...
        show(config) {
            ...
            // Initialize form fields (reference pickers, etc.) if MobileForms is available
            if (window.MobileForms && popup.body) {
                setTimeout(() => MobileForms.initFormFields(popup.body), 50);
            }
        ...
        _createPopupDOM(popup) {
            ...
            const titleHtml = config.titleHtml ||
                `<span class="mobile-popup-title">${MobileUtils.escapeHtml(config.title)}</span>`;
                    ...
                    <div class="mobile-popup-tabs">
                        ${config.tabs.map((tab, idx) => `
                            <button class="mobile-popup-tab ${idx === 0 ? 'active' : ''}"
                                    data-tab="${tab.id}">${MobileUtils.escapeHtml(tab.label)}</button>
                        `).join('')}
                    </div>
                `;
                    ...
                    <div class="mobile-popup-footer">
                        ${config.showCancelButton ? `
                            <button class="mobile-popup-btn mobile-popup-btn-cancel">
                                ${MobileUtils.escapeHtml(config.cancelButtonText)}
                            </button>
                        ` : ''}
                        <button class="mobile-popup-btn ${config.saveButtonClass}">
                            ${MobileUtils.escapeHtml(config.saveButtonText)}
                        </button>
                    </div>
                `;
            }
        ...
        showLoading(message = 'Loading...', id) {
            this.updateContent(`
                <div class="mobile-popup-loading">
                    ...
                    <span>${MobileUtils.escapeHtml(message)}</span>
                </div>
            `, id);
        }
    };
