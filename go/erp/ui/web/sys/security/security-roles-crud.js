...
(function() {
    ...
    var ACTION_NAMES = Security.enums.ACTION_NAMES;
    var tempRules = [];
    ...
    function generateRulesListHtml() {
        ...
        return tempRules.map(function(rule, index) {
            ...
            return '<div class="sys-rule-row">' +
                '<div class="sys-rule-info">' +
                '<div class="sys-rule-id">' + ERPUtils.escapeHtml(rule.ruleId) + '</div>' +
                '<div class="sys-rule-details">' +
                '<span class="' + (rule.allowed ? 'sys-tag-allow' : 'sys-tag-deny') + '">' + (rule.allowed ? 'Allow' : 'Deny') + '</span> ' +
                'Type: ' + ERPUtils.escapeHtml(rule.elemType) + ' | Actions: ' + (actionNames || 'None') +
                '</div></div>' +
    ...
    function generateRoleFormHtml(roleId, roleName) {
        ...
        return '<div class="form-group">' +
            '<label for="sys-role-id">Role ID</label>' +
            '<input type="text" id="sys-role-id" name="sys-role-id" value="' + ERPUtils.escapeAttr(roleId || '') + '"' + (isEdit ? ' disabled' : '') + ' required>' +
            '</div>' +
            '<div class="form-group">' +
            '<label for="sys-role-name">Role Name</label>' +
            '<input type="text" id="sys-role-name" name="sys-role-name" value="' + ERPUtils.escapeAttr(roleName || '') + '" required>' +
            '</div>' +
            '<div class="form-group">' +
            ...
            '</div>' +
            '<div id="sys-rules-container">' + generateRulesListHtml() + '</div>' +
            '</div>';
    }
    ...
    async function openEdit(service, roleId) {
        ...
        var role = null;
        try {
            role = await ERPForms.fetchRecord(service.endpoint, 'roleId', roleId, 'L8Role');
        } catch (e) {
            ERPNotification.error('Failed to load role');
            return;
        }
        if (!role) {
            ERPNotification.error('Role not found');
            return;
        }
    ...
    async function handleRoleSave() {
        ...
        if (!roleId || !roleName) {
            ERPNotification.warning('Please fill in Role ID and Role Name');
            return;
        }
        ...
        try {
            await ERPForms.saveRecord(currentService.endpoint, roleData, currentIsEdit);
            ERPPopup.close();
            SYS.refreshCurrentTable();
        } catch (e) {
            ERPNotification.error('Error saving role: ' + e.message);
        }
    }
    ...
    function generateActionsHtml() {
        ...
        return keys.map(function(action) {
            ...
            var options = Object.entries(ACTION_NAMES).map(function(entry) {
                return '<option value="' + ERPUtils.escapeAttr(entry[0]) + '"' + (action === entry[0] ? ' selected' : '') + '>' + entry[1] + '</option>';
            }).join('');
            return '<div class="sys-kv-row">' +
                '<select name="sys-action-' + ERPUtils.escapeAttr(action) + '">' +
                '<option value="">Select Action</option>' + options + '</select>' +
                '<button type="button" class="sys-remove-btn" data-action="remove-action" data-key="' + ERPUtils.escapeAttr(action) + '">X</button>' +
                '</div>';
        }).join('');
    }
    ...
    function generateAttributesHtml() {
        ...
        return keys.map(function(key, idx) {
            return '<div class="sys-kv-row">' +
                '<input type="text" name="sys-attr-key-' + idx + '" placeholder="Key" value="' + ERPUtils.escapeAttr(key) + '">' +
                '<input type="text" name="sys-attr-val-' + idx + '" placeholder="Value" value="' + ERPUtils.escapeAttr(tempRuleAttributes[key]) + '">' +
                '<button type="button" class="sys-remove-btn" data-action="remove-attr" data-key="' + ERPUtils.escapeAttr(key) + '">X</button>' +
                '</div>';
        }).join('');
    }
    ...
    async function generateRuleFormHtml(rule) {
        ...
        var elemTypeOptions = types.map(function(type) {
            ...
            return '<option value="' + ERPUtils.escapeAttr(type) + '"' + selected + '>' + (type === '*' ? '* (Wildcard)' : ERPUtils.escapeHtml(type)) + '</option>';
        }).join('');

        return '<div class="form-group">' +
            '<label for="sys-rule-id">Rule ID</label>' +
            '<input type="text" id="sys-rule-id" name="sys-rule-id" value="' + ERPUtils.escapeAttr(ruleId) + '" required>' +
            '</div>' +
            '<div class="form-group">' +
            '<label for="sys-rule-elem-type">Element Type</label>' +
            '<select id="sys-rule-elem-type" name="sys-rule-elem-type">' + elemTypeOptions + '</select>' +
            '</div>' +
            '<div class="form-group">' +
            '<label for="sys-rule-allowed">Rule Type</label>' +
            '<select id="sys-rule-allowed" name="sys-rule-allowed">' +
            '<option value="true"' + (allowed ? ' selected' : '') + '>Allow</option>' +
            '<option value="false"' + (!allowed ? ' selected' : '') + '>Deny</option>' +
            '</select></div>' +
            '<div class="form-group">' +
            '<div class="sys-nested-table-header"><span>Actions</span>' +
            '<button type="button" class="btn btn-small" data-action="add-action">Add Action</button></div>' +
            '<div id="sys-actions-container">' + generateActionsHtml() + '</div>' +
            '</div>' +
            '<div class="form-group">' +
            '<div class="sys-nested-table-header"><span>Attributes</span>' +
            '<button type="button" class="btn btn-small" data-action="add-attr">Add Attribute</button></div>' +
            '<div id="sys-attrs-container">' + generateAttributesHtml() + '</div>' +
            '</div>';
    }
    ...
    function handleRuleSave() {
        ...
        if (!ruleId || !elemType) {
            ERPNotification.warning('Please fill in Rule ID and Element Type');
            return;
        }

        // Collect actions from selects
        var actions = {};
        body.querySelectorAll('select[name^="sys-action-"]').forEach(function(sel) {
            if (sel.value) actions[sel.value] = true;
        });

        // Collect attributes
        var attributes = {};
        var attrKeys = body.querySelectorAll('input[name^="sys-attr-key-"]');
        attrKeys.forEach(function(keyInput) {
            var idx = keyInput.name.replace('sys-attr-key-', '');
            var valInput = body.querySelector('input[name="sys-attr-val-' + idx + '"]');
        ...
        if (currentEditingRuleIndex !== null && currentEditingRuleIndex !== undefined) {
            ...
            if (exists >= 0) {
                ERPNotification.warning('Rule ID already exists in this role');
                return;
            }
            tempRules.push(rule);
        }
    ...
    function confirmDelete(service, roleId) {
        ...
        ERPForms.confirmDelete(serviceConfig, roleId, function() {
            SYS.refreshCurrentTable();
        });
    }
