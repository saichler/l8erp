...
(function() {
    ...
    // Open add modal
    SCM._openAddModal = function(service) {
        const formDef = SCM.getServiceFormDef(service.model);

        if (!formDef || typeof SCMForms === 'undefined') {
        ...
        const serviceConfig = {
            ...
            primaryKey: SCM.getServicePrimaryKey(service.model),
            modelName: service.model
        };

        SCMForms.openAddForm(serviceConfig, formDef, () => {
            SCM.refreshCurrentTable();
        });
    };

    // Open edit modal
    SCM._openEditModal = function(service, id) {
        const formDef = SCM.getServiceFormDef(service.model);
        ...
        SCMForms.openEditForm(serviceConfig, formDef, id, () => {
            SCM.refreshCurrentTable();
        });
    };

    // Confirm delete
    SCM._confirmDeleteItem = function(service, id) {
        const serviceConfig = {
            endpoint: service.endpoint,
            primaryKey: SCM.getServicePrimaryKey(service.model),
            modelName: service.model
        ...
        if (typeof SCMForms !== 'undefined') {
            SCMForms.confirmDelete(serviceConfig, id, () => {
                SCM.refreshCurrentTable();
            });
        } else if (confirm(`Are you sure you want to delete this ${service.label.replace(/s$/, '')}?`)) {
            SCM._deleteItem(service, id);
        }
    };

    // Delete item
    SCM._deleteItem = async function(service, id) {
        try {
            ...
            // Refresh table
            const tableId = `${SCM._state.currentModule}-${service.key}-table`;
    ...
    SCM._showDetailsModal = function(service, item, itemId) {
        const formDef = SCM.getServiceFormDef(service.model);
        ...
        let content = SCMForms.generateFormHtml(formDef, item);
    ...
    SCM._getFieldDisplayValue = function(item, field) {
    ...
    SCM._formatFieldLabel = function(key) {
        return key
            .replace(/([A-Z])/g, ' $1')
    ...
    SCM._formatFieldValue = function(key, value) {
        if (value === null || value === undefined || value === '') {
            return '<span class="detail-empty">-</span>';
        }
