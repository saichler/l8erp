...
(function() {
     ...
     */
    ERPReferencePicker.attach = function(inputElement, options = {}) {
        // Validate required options
        if (!options.endpoint) {
            console.error('ERPReferencePicker: endpoint is required');
            return;
        }
        if (!options.modelName) {
            console.error('ERPReferencePicker: modelName is required');
            return;
        }
        if (!options.idColumn) {
            console.error('ERPReferencePicker: idColumn is required');
            return;
        }
        if (!options.displayColumn) {
            console.error('ERPReferencePicker: displayColumn is required');
            return;
        }
        ...
        inputElement.classList.add('erp-refpicker-input');
     ...
     */
    ERPReferencePicker.open = async function(inputElement) {
        const config = internal.attachedInputs.get(inputElement);
        if (!config) {
            console.warn('ERPReferencePicker: Input not attached - no config found');
            return;
        }
     ...
     */
    ERPReferencePicker.close = function() {
     ...
     */
    ERPReferencePicker.getValue = function(inputElement) {
     ...
     */
    ERPReferencePicker.getItem = function(inputElement) {
        if (inputElement.dataset.refItem) {
            try {
     ...
     */
    ERPReferencePicker.setValue = function(inputElement, id, displayValue, item = null) {
     ...
     */
    ERPReferencePicker.detach = function(inputElement) {
        ...
        inputElement.classList.remove('erp-refpicker-input');
    };
