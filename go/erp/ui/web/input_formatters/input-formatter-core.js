...
(function() {
    ...
    const { utils, masks, getType, hasType } = ERPInputFormatter;
    ...
    function attach(input, type, options = {}) {
        if (!input || !(input instanceof HTMLElement)) {
            console.error('ERPInputFormatter.attach: Invalid input element');
            return false;
        }

        if (!hasType(type)) {
            console.error(`ERPInputFormatter.attach: Unknown type "${type}"`);
            return false;
        }

        // Don't attach twice
        if (utils.hasFormatter(input)) {
            console.warn('ERPInputFormatter.attach: Input already has a formatter attached');
            return false;
        }
