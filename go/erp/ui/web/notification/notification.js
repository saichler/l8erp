...
(function() {
    ...
    function ensureContainer() {
        ...
        container.className = 'erp-notification-container';
    ...
    function createNotificationElement(config) {
        ...
        const detailsHtml = config.details && config.details.length > 0
            ? `<ul class="erp-notification-details">
                ${config.details.map(d => `<li>${escapeHtml(d)}</li>`).join('')}
               </ul>`
            : '';

        const closeButtonHtml = config.showClose
            ? `<button class="erp-notification-close" aria-label="Close notification">${ICONS.close}</button>`
            : '';

        el.innerHTML = `
            <div class="erp-notification-header">
                <div class="erp-notification-icon">${ICONS[config.type]}</div>
                <h4 class="erp-notification-title">${escapeHtml(config.title)}</h4>
                ${closeButtonHtml}
            </div>
            <div class="erp-notification-body">
                <p class="erp-notification-message">${escapeHtml(config.message)}</p>
                ${detailsHtml}
            </div>
        `;
    ...
    function setupEventHandlers(element, config) {
        // Close button click
        const closeBtn = element.querySelector('.erp-notification-close');
        if (closeBtn) {
    ...
    function close() {
        ...
        currentNotification.classList.add('erp-notification-closing');
