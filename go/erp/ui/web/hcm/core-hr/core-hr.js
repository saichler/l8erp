...
function escapeHtmlInRender(text) {
...
function renderEmploymentStatus(status) {
    const statusClasses = {
        1: 'hcm-status-active',
        2: 'hcm-status-inactive',
        3: 'hcm-status-pending',
        4: 'hcm-status-terminated',
        5: 'hcm-status-inactive',
        6: 'hcm-status-terminated',
        7: 'hcm-status-pending'
    };
    const label = EMPLOYMENT_STATUS[status] || 'Unknown';
    const cssClass = statusClasses[status] || '';
    return `<span class="hcm-status ${cssClass}">${label}</span>`;
}

function renderEmploymentType(type) {
    return EMPLOYMENT_TYPE[type] || 'Unknown';
}

function renderOrgType(type) {
    return ORGANIZATION_TYPE[type] || 'Unknown';
}

function renderPositionStatus(status) {
    const statusClasses = {
    ...
    const label = POSITION_STATUS[status] || 'Unknown';
    const cssClass = statusClasses[status] || '';
    return `<span class="hcm-status ${cssClass}">${label}</span>`;
}

function renderDocumentType(type) {
    return DOCUMENT_TYPE[type] || 'Unknown';
}

function renderComplianceType(type) {
    return COMPLIANCE_TYPE[type] || 'Unknown';
}

function renderBoolean(value, trueText = 'Yes', falseText = 'No') {
...
const CORE_HR_COLUMNS = {
    Employee: [
        ...
        { key: 'employeeNumber', label: 'Emp #', sortKey: 'employeeNumber', filterKey: 'employeeNumber' },
        {
            ...
            render: (item) => {
                ...
                return `<a href="#" class="emp-name-link" onclick="EmployeeDetail.open('${item.employeeId}'); return false;">${escapeHtmlInRender(name)}</a>`;
            }
        },
        {
    ...
    Job: [
        ...
        { key: 'jobLevel', label: 'Level', sortKey: 'jobLevel', filterKey: 'jobLevel' },
        {
            ...
            render: (item) => renderBoolean(item.isFlsaExempt, 'Exempt', 'Non-Exempt')
        },
        {
            key: 'isActive',
            label: 'Active',
