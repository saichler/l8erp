...
const GARNISHMENT_STATUS_VALUES = {
    'active': 1,
    'suspended': 2,
    'completed': 3,
    'cancelled': 4
...
const YEAR_END_DOC_STATUS_VALUES = {
    'draft': 1,
    'generated': 2,
    'issued': 3,
    'filed': 4
...
function escapeHtmlPayroll(text) {
...
function renderComponentType(type) {
    const typeClasses = {
        1: 'hcm-status-active',      // Earning - green
        2: 'hcm-status-terminated',  // Deduction - red
        3: 'hcm-status-pending',     // Tax - orange
        4: 'hcm-status-inactive'     // Employer - gray
    };
    const label = PAY_COMPONENT_TYPE[type] || 'Unknown';
    const cssClass = typeClasses[type] || '';
    return `<span class="hcm-status ${cssClass}">${label}</span>`;
}

function renderComponentCategory(cat) {
    return PAY_COMPONENT_CATEGORY[cat] || 'Unknown';
}

function renderCalculationType(type) {
    return CALCULATION_TYPE[type] || 'Unknown';
}

function renderPayrollRunStatus(status) {
    const statusClasses = {
        1: 'hcm-status-inactive',    // Draft
        2: 'hcm-status-pending',     // Pending Approval
        3: 'hcm-status-active',      // Approved
        4: 'hcm-status-pending',     // Processing
        5: 'hcm-status-active',      // Completed
        6: 'hcm-status-terminated',  // Cancelled
        7: 'hcm-status-terminated'   // Error
    };
    const label = PAYROLL_RUN_STATUS[status] || 'Unknown';
    const cssClass = statusClasses[status] || '';
    return `<span class="hcm-status ${cssClass}">${label}</span>`;
}

function renderPayrollRunType(type) {
    return PAYROLL_RUN_TYPE[type] || 'Unknown';
}

function renderTaxType(type) {
    return TAX_TYPE[type] || 'Unknown';
}

function renderFilingStatus(status) {
    return FILING_STATUS[status] || 'Unknown';
}

function renderAccountType(type) {
    return ACCOUNT_TYPE[type] || 'Unknown';
}

function renderDepositType(type) {
    return DEPOSIT_TYPE[type] || 'Unknown';
}

function renderGarnishmentType(type) {
    return GARNISHMENT_TYPE[type] || 'Unknown';
}

function renderGarnishmentStatus(status) {
    const statusClasses = {
        1: 'hcm-status-active',
        2: 'hcm-status-pending',
        3: 'hcm-status-inactive',
        4: 'hcm-status-terminated'
    };
    const label = GARNISHMENT_STATUS[status] || 'Unknown';
    const cssClass = statusClasses[status] || '';
    return `<span class="hcm-status ${cssClass}">${label}</span>`;
}

function renderYearEndDocType(type) {
    return YEAR_END_DOC_TYPE[type] || 'Unknown';
}

function renderYearEndDocStatus(status) {
    const statusClasses = {
        1: 'hcm-status-inactive',    // Draft
        2: 'hcm-status-pending',     // Generated
        3: 'hcm-status-active',      // Issued
        4: 'hcm-status-active'       // Filed
    };
    const label = YEAR_END_DOC_STATUS[status] || 'Unknown';
    const cssClass = statusClasses[status] || '';
    return `<span class="hcm-status ${cssClass}">${label}</span>`;
}
...
function renderBooleanPayroll(value, trueText = 'Yes', falseText = 'No') {
    const cssClass = value ? 'hcm-status-active' : 'hcm-status-inactive';
    const label = value ? trueText : falseText;
    return `<span class="hcm-status ${cssClass}">${label}</span>`;
}

function renderDatePayroll(timestamp) {
    if (!timestamp) return '-';
...
const PAYROLL_COLUMNS = {
    PayStructure: [
        ...
        },
        {
            ...
            label: 'Active',
            sortKey: 'isActive',
            render: (item) => renderBooleanPayroll(item.isActive)
        }
    ],
    ...
    PayComponent: [
        ...
        { key: 'name', label: 'Name', sortKey: 'name', filterKey: 'name' },
        {
            ...
            filterKey: 'componentType',
            enumValues: PAY_COMPONENT_TYPE_VALUES,
            render: (item) => renderComponentType(item.componentType)
        },
        {
            key: 'calculationType',
            label: 'Calculation',
            ...
            enumValues: CALCULATION_TYPE_VALUES,
            render: (item) => renderCalculationType(item.calculationType)
        },
        {
            key: 'isTaxable',
            label: 'Taxable',
            sortKey: 'isTaxable',
            render: (item) => renderBooleanPayroll(item.isTaxable)
        },
        {
            key: 'isActive',
            label: 'Active',
    ...
    PayrollRun: [
        ...
        { key: 'name', label: 'Name', sortKey: 'name', filterKey: 'name' },
        {
            ...
            sortKey: 'runType',
            filterKey: 'runType',
            enumValues: PAYROLL_RUN_TYPE_VALUES,
            render: (item) => renderPayrollRunType(item.runType)
        },
        {
            key: 'status',
            label: 'Status',
            sortKey: 'status',
            filterKey: 'status',
            enumValues: PAYROLL_RUN_STATUS_VALUES,
            render: (item) => renderPayrollRunStatus(item.status)
        },
        {
            key: 'paymentDate',
            label: 'Payment Date',
            sortKey: 'paymentDate',
            render: (item) => renderDatePayroll(item.paymentDate)
        },
        { key: 'employeeCount', label: 'Employees', sortKey: 'employeeCount' },
        {
            ...
            label: 'Total Net',
            sortKey: 'totalNet',
            render: (item) => renderMoney(item.totalNet)
        }
    ],
    ...
    Payslip: [
        ...
        },
        {
            ...
            label: 'Gross',
            sortKey: 'grossPay',
            render: (item) => renderMoney(item.grossPay)
        },
        {
            key: 'totalDeductions',
            label: 'Deductions',
            sortKey: 'totalDeductions',
            render: (item) => renderMoney(item.totalDeductions)
        },
        {
            key: 'netPay',
            label: 'Net Pay',
            sortKey: 'netPay',
            render: (item) => renderMoney(item.netPay)
        }
    ],
    ...
    TaxWithholding: [
        ...
        { key: 'employeeId', label: 'Employee', sortKey: 'employeeId', filterKey: 'employeeId' },
        {
            ...
            sortKey: 'taxType',
            filterKey: 'taxType',
            enumValues: TAX_TYPE_VALUES,
            render: (item) => renderTaxType(item.taxType)
        },
        { key: 'taxJurisdiction', label: 'Jurisdiction', sortKey: 'taxJurisdiction', filterKey: 'taxJurisdiction' },
        {
            key: 'filingStatus',
            label: 'Filing Status',
            sortKey: 'filingStatus',
            filterKey: 'filingStatus',
            enumValues: FILING_STATUS_VALUES,
            render: (item) => renderFilingStatus(item.filingStatus)
        },
        {
            key: 'exempt',
            label: 'Exempt',
            sortKey: 'exempt',
            render: (item) => renderBooleanPayroll(item.exempt)
        }
    ],
    ...
    DirectDeposit: [
        ...
        { key: 'accountNumberMasked', label: 'Account', sortKey: 'accountNumberMasked' },
        {
            ...
            render: (item) => renderAccountType(item.accountType)
        },
        {
            key: 'depositType',
            label: 'Deposit',
            ...
            enumValues: DEPOSIT_TYPE_VALUES,
            render: (item) => renderDepositType(item.depositType)
        },
        {
            key: 'isActive',
            label: 'Active',
    ...
    Garnishment: [
        ...
        { key: 'employeeId', label: 'Employee', sortKey: 'employeeId', filterKey: 'employeeId' },
        {
            key: 'garnishmentType',
            label: 'Type',
            sortKey: 'garnishmentType',
            filterKey: 'garnishmentType',
            enumValues: GARNISHMENT_TYPE_VALUES,
            render: (item) => renderGarnishmentType(item.garnishmentType)
        },
        ...
        { key: 'payeeName', label: 'Payee', sortKey: 'payeeName', filterKey: 'payeeName' },
        {
            ...
            sortKey: 'status',
            filterKey: 'status',
            enumValues: GARNISHMENT_STATUS_VALUES,
            render: (item) => renderGarnishmentStatus(item.status)
        },
        {
            key: 'amountPerPeriod',
            label: 'Per Period',
            sortKey: 'amountPerPeriod',
            render: (item) => renderMoney(item.amountPerPeriod)
        }
    ],
    ...
    YearEndDocument: [
        ...
        { key: 'taxYear', label: 'Tax Year', sortKey: 'taxYear', filterKey: 'taxYear' },
        {
            ...
            enumValues: YEAR_END_DOC_TYPE_VALUES,
            render: (item) => renderYearEndDocType(item.documentType)
        },
        {
            key: 'status',
            label: 'Status',
            sortKey: 'status',
            filterKey: 'status',
            enumValues: YEAR_END_DOC_STATUS_VALUES,
            render: (item) => renderYearEndDocStatus(item.status)
        },
        {
            key: 'issuedDate',
            label: 'Issued',
            sortKey: 'issuedDate',
            render: (item) => renderDatePayroll(item.issuedDate)
        },
        {
            key: 'isCorrected',
            label: 'Corrected',
            sortKey: 'isCorrected',
            render: (item) => renderBooleanPayroll(item.isCorrected)
        }
    ]
};
...
if (typeof window !== 'undefined') {
    window.Payroll = {
        ...
        enums: {
            ...
            PAY_COMPONENT_TYPE,
            PAY_COMPONENT_TYPE_VALUES,
            PAY_COMPONENT_CATEGORY,
            CALCULATION_TYPE,
            CALCULATION_TYPE_VALUES,
            PAYROLL_RUN_STATUS,
            PAYROLL_RUN_STATUS_VALUES,
            PAYROLL_RUN_TYPE,
            PAYROLL_RUN_TYPE_VALUES,
            TAX_TYPE,
            TAX_TYPE_VALUES,
            FILING_STATUS,
            ...
            DEPOSIT_TYPE_VALUES,
            GARNISHMENT_TYPE,
            GARNISHMENT_TYPE_VALUES,
            GARNISHMENT_STATUS,
            GARNISHMENT_STATUS_VALUES,
            YEAR_END_DOC_TYPE,
            YEAR_END_DOC_TYPE_VALUES,
            YEAR_END_DOC_STATUS,
            YEAR_END_DOC_STATUS_VALUES
        },
        render: {
            payFrequency: renderPayFrequency,
            componentType: renderComponentType,
            componentCategory: renderComponentCategory,
            calculationType: renderCalculationType,
            payrollRunStatus: renderPayrollRunStatus,
            payrollRunType: renderPayrollRunType,
            taxType: renderTaxType,
            filingStatus: renderFilingStatus,
            accountType: renderAccountType,
            depositType: renderDepositType,
            garnishmentType: renderGarnishmentType,
            garnishmentStatus: renderGarnishmentStatus,
            yearEndDocType: renderYearEndDocType,
            yearEndDocStatus: renderYearEndDocStatus,
            money: renderMoney,
            boolean: renderBooleanPayroll,
            date: renderDatePayroll,
