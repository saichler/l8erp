...
function escapeHtmlTalent(text) {
...
function renderRequisitionStatus(status) {
    const statusClasses = {
        1: 'hcm-status-inactive',    // Draft
        2: 'hcm-status-pending',     // Pending Approval
        3: 'hcm-status-active',      // Approved
        4: 'hcm-status-active',      // Open
        5: 'hcm-status-pending',     // On Hold
        6: 'hcm-status-active',      // Filled
        7: 'hcm-status-terminated',  // Cancelled
        8: 'hcm-status-inactive'     // Closed
    };
    const label = REQUISITION_STATUS[status] || 'Unknown';
    const cssClass = statusClasses[status] || '';
    return `<span class="hcm-status ${cssClass}">${label}</span>`;
}

function renderRequisitionType(type) {
    return REQUISITION_TYPE[type] || 'Unknown';
}

function renderApplicantSource(source) {
    return APPLICANT_SOURCE[source] || 'Unknown';
}

function renderApplicationStatus(status) {
    const statusClasses = {
        1: 'hcm-status-pending',     // New
        2: 'hcm-status-pending',     // In Review
        3: 'hcm-status-active',      // Interviewing
        4: 'hcm-status-pending',     // Offer Pending
        5: 'hcm-status-active',      // Offer Extended
        6: 'hcm-status-active',      // Hired
        7: 'hcm-status-terminated',  // Rejected
        8: 'hcm-status-inactive'     // Withdrawn
    };
    const label = APPLICATION_STATUS[status] || 'Unknown';
    const cssClass = statusClasses[status] || '';
    return `<span class="hcm-status ${cssClass}">${label}</span>`;
}

function renderApplicationStage(stage) {
    return APPLICATION_STAGE[stage] || 'Unknown';
}

function renderDispositionReason(reason) {
    return DISPOSITION_REASON[reason] || 'Unknown';
}

function renderOnboardingTaskCategory(cat) {
    return ONBOARDING_TASK_CATEGORY[cat] || 'Unknown';
}

function renderTaskOwner(owner) {
    return TASK_OWNER[owner] || 'Unknown';
}

function renderOnboardingTaskStatus(status) {
    const statusClasses = {
        1: 'hcm-status-inactive',    // Not Started
        2: 'hcm-status-pending',     // In Progress
        3: 'hcm-status-active',      // Completed
        4: 'hcm-status-inactive',    // Skipped
        5: 'hcm-status-terminated'   // Blocked
    };
    const label = ONBOARDING_TASK_STATUS[status] || 'Unknown';
    const cssClass = statusClasses[status] || '';
    return `<span class="hcm-status ${cssClass}">${label}</span>`;
}

function renderPerformanceReviewStatus(status) {
    const statusClasses = {
        1: 'hcm-status-inactive',    // Not Started
        2: 'hcm-status-pending',     // Self Review
        3: 'hcm-status-pending',     // Manager Review
        4: 'hcm-status-pending',     // Calibration
        5: 'hcm-status-pending',     // HR Review
        6: 'hcm-status-pending',     // Acknowledgment
        7: 'hcm-status-active'       // Completed
    };
    const label = PERFORMANCE_REVIEW_STATUS[status] || 'Unknown';
    const cssClass = statusClasses[status] || '';
    return `<span class="hcm-status ${cssClass}">${label}</span>`;
}

function renderReviewType(type) {
    return REVIEW_TYPE[type] || 'Unknown';
}

function renderGoalType(type) {
    return GOAL_TYPE[type] || 'Unknown';
}

function renderGoalCategory(cat) {
    return GOAL_CATEGORY[cat] || 'Unknown';
}
...
function renderGoalStatus(status) {
    const statusClasses = {
        ...
        3: 'hcm-status-active',      // On Track
        4: 'hcm-status-pending',     // At Risk
        5: 'hcm-status-terminated',  // Behind
        6: 'hcm-status-active',      // Completed
        7: 'hcm-status-inactive'     // Cancelled
    };
    const label = GOAL_STATUS[status] || 'Unknown';
    const cssClass = statusClasses[status] || '';
    return `<span class="hcm-status ${cssClass}">${label}</span>`;
}
...
function renderSuccessionPlanStatus(status) {
    const statusClasses = {
        1: 'hcm-status-inactive',    // Draft
        2: 'hcm-status-active',      // Active
        3: 'hcm-status-pending',     // On Hold
        4: 'hcm-status-inactive'     // Closed
    };
    const label = SUCCESSION_PLAN_STATUS[status] || 'Unknown';
    const cssClass = statusClasses[status] || '';
    return `<span class="hcm-status ${cssClass}">${label}</span>`;
}
...
function renderFeedbackType(type) {
    return FEEDBACK_TYPE[type] || 'Unknown';
}

function renderFeedbackRelationship(rel) {
    return FEEDBACK_RELATIONSHIP[rel] || 'Unknown';
}

function renderFeedbackStatus(status) {
    const statusClasses = {
        1: 'hcm-status-pending',     // Requested
        2: 'hcm-status-pending',     // In Progress
        3: 'hcm-status-active',      // Submitted
        4: 'hcm-status-terminated'   // Declined
    };
    const label = FEEDBACK_STATUS[status] || 'Unknown';
    const cssClass = statusClasses[status] || '';
    return `<span class="hcm-status ${cssClass}">${label}</span>`;
}

function renderMoneyTalent(money) {
...
function renderBooleanTalent(value, trueText = 'Yes', falseText = 'No') {
    const cssClass = value ? 'hcm-status-active' : 'hcm-status-inactive';
    const label = value ? trueText : falseText;
    return `<span class="hcm-status ${cssClass}">${label}</span>`;
}

function renderDateTalent(timestamp) {
    if (!timestamp) return '-';
...
const TALENT_COLUMNS = {
    PerformanceReview: [
        ...
        { key: 'reviewerId', label: 'Reviewer', sortKey: 'reviewerId', filterKey: 'reviewerId' },
        {
            ...
            render: (item) => renderReviewType(item.reviewType)
        },
        {
            key: 'reviewPeriod',
            label: 'Period',
    ...
    Goal: [
        ...
        { key: 'title', label: 'Title', sortKey: 'title', filterKey: 'title' },
        {
            ...
            render: (item) => renderGoalType(item.goalType)
        },
        {
            key: 'priority',
            label: 'Priority',
        ...
        },
        {
            ...
            label: 'Due Date',
            sortKey: 'dueDate',
            render: (item) => renderDateTalent(item.dueDate)
        }
    ],
    ...
    Feedback: [
        ...
        { key: 'providerId', label: 'Provider', sortKey: 'providerId', filterKey: 'providerId' },
        {
            ...
            render: (item) => renderFeedbackType(item.feedbackType)
        },
        {
            key: 'relationship',
            label: 'Relationship',
            ...
            render: (item) => renderFeedbackRelationship(item.relationship)
        },
        {
            key: 'status',
            label: 'Status',
    ...
    CareerPath: [
        ...
        },
        {
            ...
            label: 'Active',
            sortKey: 'isActive',
            render: (item) => renderBooleanTalent(item.isActive)
        }
    ],
    ...
    SuccessionPlan: [
        ...
        },
        {
            ...
            render: (item) => renderDateTalent(item.nextReviewDate)
        }
    ],
    ...
    JobRequisition: [
        ...
        { key: 'title', label: 'Title', sortKey: 'title', filterKey: 'title' },
        {
            ...
            render: (item) => renderRequisitionType(item.requisitionType)
        },
        {
            key: 'status',
            label: 'Status',
        ...
        { key: 'applicantCount', label: 'Applicants', sortKey: 'applicantCount' },
        {
            ...
            render: (item) => renderDateTalent(item.targetStartDate)
        }
    ],
    ...
    Applicant: [
        { key: 'applicantId', label: 'ID', sortKey: 'applicantId', filterKey: 'applicantId' },
        {
            ...
            sortKey: 'lastName',
            filterKey: 'lastName',
            render: (item) => `${escapeHtmlTalent(item.firstName || '')} ${escapeHtmlTalent(item.lastName || '')}`.trim()
        },
        ...
        { key: 'phone', label: 'Phone', sortKey: 'phone', filterKey: 'phone' },
        {
            ...
            render: (item) => renderApplicantSource(item.source)
        },
        {
            key: 'createdDate',
            label: 'Applied',
            sortKey: 'createdDate',
            render: (item) => renderDateTalent(item.createdDate)
        }
    ],
    ...
    Application: [
        ...
        },
        {
            ...
            render: (item) => renderApplicationStage(item.stage)
        },
        {
            key: 'overallRating',
            label: 'Rating',
        ...
        },
        {
            ...
            label: 'Applied',
            sortKey: 'appliedDate',
            render: (item) => renderDateTalent(item.appliedDate)
        }
    ],
    ...
    OnboardingTask: [
        ...
        { key: 'name', label: 'Task', sortKey: 'name', filterKey: 'name' },
        {
            ...
            render: (item) => renderOnboardingTaskCategory(item.category)
        },
        {
            key: 'ownerType',
            label: 'Owner',
            ...
            render: (item) => renderTaskOwner(item.ownerType)
        },
        {
            key: 'status',
            label: 'Status',
...
if (typeof window !== 'undefined') {
    window.Talent = {
        ...
        render: {
            ...
            requisitionType: renderRequisitionType,
            applicantSource: renderApplicantSource,
            applicationStatus: renderApplicationStatus,
            applicationStage: renderApplicationStage,
            dispositionReason: renderDispositionReason,
            onboardingTaskCategory: renderOnboardingTaskCategory,
            taskOwner: renderTaskOwner,
            onboardingTaskStatus: renderOnboardingTaskStatus,
            performanceReviewStatus: renderPerformanceReviewStatus,
            reviewType: renderReviewType,
            goalType: renderGoalType,
            goalCategory: renderGoalCategory,
            ...
            feedbackType: renderFeedbackType,
            feedbackRelationship: renderFeedbackRelationship,
            feedbackStatus: renderFeedbackStatus,
            money: renderMoneyTalent,
            boolean: renderBooleanTalent,
            date: renderDateTalent,
