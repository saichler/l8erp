...
function renderTimesheetStatus(status) {
    const statusClasses = {
        ...
        4: 'hcm-status-terminated',  // Rejected
        5: 'hcm-status-active'       // Processed
    };
    const label = TIMESHEET_STATUS[status] || 'Unknown';
    const cssClass = statusClasses[status] || '';
    return `<span class="hcm-status ${cssClass}">${label}</span>`;
}

function renderTimeEntryType(type) {
    return TIME_ENTRY_TYPE[type] || 'Unknown';
}

function renderTimeEntrySource(source) {
    return TIME_ENTRY_SOURCE[source] || 'Unknown';
}

function renderLeaveType(type) {
    return LEAVE_TYPE[type] || 'Unknown';
}

function renderLeaveRequestStatus(status) {
    const statusClasses = {
        1: 'hcm-status-inactive',    // Draft
        2: 'hcm-status-pending',     // Pending
        3: 'hcm-status-active',      // Approved
        4: 'hcm-status-terminated',  // Rejected
        5: 'hcm-status-terminated',  // Cancelled
        6: 'hcm-status-active'       // Taken
    };
    const label = LEAVE_REQUEST_STATUS[status] || 'Unknown';
    const cssClass = statusClasses[status] || '';
    return `<span class="hcm-status ${cssClass}">${label}</span>`;
}

function renderAccrualMethod(method) {
    return ACCRUAL_METHOD[method] || 'Unknown';
}

function renderAccrualFrequency(freq) {
    return ACCRUAL_FREQUENCY[freq] || 'Unknown';
}
...
function renderScheduleStatus(status) {
    const statusClasses = {
        1: 'hcm-status-inactive',    // Draft
        2: 'hcm-status-active',      // Published
        3: 'hcm-status-inactive'     // Archived
    };
    const label = SCHEDULE_STATUS[status] || 'Unknown';
    const cssClass = statusClasses[status] || '';
    return `<span class="hcm-status ${cssClass}">${label}</span>`;
}

function renderHolidayType(type) {
    return HOLIDAY_TYPE[type] || 'Unknown';
}

function renderAbsenceStatus(status) {
    const statusClasses = {
        1: 'hcm-status-pending',     // Scheduled
        2: 'hcm-status-active',      // Taken
        3: 'hcm-status-terminated'   // Cancelled
    };
    const label = ABSENCE_STATUS[status] || 'Unknown';
    const cssClass = statusClasses[status] || '';
    return `<span class="hcm-status ${cssClass}">${label}</span>`;
}

function renderMoneyTime(money) {
...
function renderBooleanTime(value, trueText = 'Yes', falseText = 'No') {
...
const TIME_COLUMNS = {
    ...
    LeaveRequest: [
        ...
        { key: 'employeeId', label: 'Employee', sortKey: 'employeeId', filterKey: 'employeeId' },
        {
            ...
            render: (item) => renderLeaveType(item.leaveType)
        },
        {
            key: 'startDate',
            label: 'Start Date',
            sortKey: 'startDate',
            render: (item) => renderDateTime(item.startDate)
        },
        {
            key: 'endDate',
            label: 'End Date',
            sortKey: 'endDate',
            render: (item) => renderDateTime(item.endDate)
        },
        {
            key: 'totalHours',
            label: 'Hours',
    ...
    LeavePolicy: [
        ...
        },
        {
            ...
            render: (item) => renderAccrualMethod(item.accrualMethod)
        },
        {
            key: 'accrualRate',
            label: 'Accrual Rate',
        ...
        },
        {
            ...
            label: 'Active',
            sortKey: 'isActive',
            render: (item) => renderBooleanTime(item.isActive)
        }
    ],
    ...
    Schedule: [
        ...
        },
        {
            ...
            render: (item) => renderDateTime(item.publishedDate)
        }
    ],
    ...
    Holiday: [
        ...
        { key: 'name', label: 'Name', sortKey: 'name', filterKey: 'name' },
        {
            ...
            label: 'Date',
            sortKey: 'date',
            render: (item) => renderDateTime(item.date)
        },
        { key: 'year', label: 'Year', sortKey: 'year', filterKey: 'year' },
        {
            key: 'holidayType',
            ...
            render: (item) => renderHolidayType(item.holidayType)
        },
        {
            key: 'isPaid',
            label: 'Paid',
            sortKey: 'isPaid',
            render: (item) => renderBooleanTime(item.isPaid)
        },
        {
            key: 'hoursCredited',
            label: 'Hours',
    ...
    Absence: [
        ...
        },
        {
            ...
            enumValues: LEAVE_TYPE_VALUES,
            render: (item) => renderLeaveType(item.absenceType)
        },
        {
            key: 'hours',
            label: 'Hours',
...
if (typeof window !== 'undefined') {
    window.Time = {
        ...
        render: {
            ...
            timeEntryType: renderTimeEntryType,
            timeEntrySource: renderTimeEntrySource,
            leaveType: renderLeaveType,
            leaveRequestStatus: renderLeaveRequestStatus,
            accrualMethod: renderAccrualMethod,
            accrualFrequency: renderAccrualFrequency,
            shiftType: renderShiftType,
            scheduleStatus: renderScheduleStatus,
            holidayType: renderHolidayType,
            absenceStatus: renderAbsenceStatus,
            money: renderMoneyTime,
            boolean: renderBooleanTime,
            date: renderDateTime,
