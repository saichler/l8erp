...
function escapeHtmlLearning(text) {
...
function renderCourseType(type) {
    return COURSE_TYPE[type] || 'Unknown';
}
...
function renderCourseCategory(cat) {
    return COURSE_CATEGORY[cat] || 'Unknown';
}
...
function renderSessionStatus(status) {
    const statusClasses = {
        ...
        3: 'hcm-status-pending',     // Full
        4: 'hcm-status-active',      // In Progress
        5: 'hcm-status-active',      // Completed
        6: 'hcm-status-terminated'   // Cancelled
    };
    const label = SESSION_STATUS[status] || 'Unknown';
    const cssClass = statusClasses[status] || '';
    return `<span class="hcm-status ${cssClass}">${label}</span>`;
}

function renderCourseEnrollmentStatus(status) {
    const statusClasses = {
        1: 'hcm-status-pending',     // Pending
        2: 'hcm-status-active',      // Active
        3: 'hcm-status-active',      // Completed
        4: 'hcm-status-terminated',  // Failed
        5: 'hcm-status-terminated',  // Cancelled
        6: 'hcm-status-pending'      // Waitlisted
    };
    const label = COURSE_ENROLLMENT_STATUS[status] || 'Unknown';
    const cssClass = statusClasses[status] || '';
    return `<span class="hcm-status ${cssClass}">${label}</span>`;
}

function renderCertificationType(type) {
    return CERTIFICATION_TYPE[type] || 'Unknown';
}

function renderCertificationStatus(status) {
    const statusClasses = {
        1: 'hcm-status-pending',     // In Progress
        2: 'hcm-status-active',      // Active
        3: 'hcm-status-terminated',  // Expired
        4: 'hcm-status-terminated',  // Revoked
        5: 'hcm-status-pending'      // Pending Renewal
    };
    const label = CERTIFICATION_STATUS[status] || 'Unknown';
    const cssClass = statusClasses[status] || '';
    return `<span class="hcm-status ${cssClass}">${label}</span>`;
}

function renderSkillCategory(cat) {
    return SKILL_CATEGORY[cat] || 'Unknown';
}

function renderTrainingType(type) {
    return TRAINING_TYPE[type] || 'Unknown';
}

function renderMoneyLearning(money) {
...
function renderBooleanLearning(value, trueText = 'Yes', falseText = 'No') {
    const cssClass = value ? 'hcm-status-active' : 'hcm-status-inactive';
    const label = value ? trueText : falseText;
    return `<span class="hcm-status ${cssClass}">${label}</span>`;
}

function renderDateLearning(timestamp) {
    if (!timestamp) return '-';
...
const LEARNING_COLUMNS = {
    Course: [
        ...
        },
        {
            ...
            label: 'Active',
            sortKey: 'isActive',
            render: (item) => renderBooleanLearning(item.isActive)
        }
    ],
    ...
    CourseSession: [
        ...
        { key: 'courseId', label: 'Course', sortKey: 'courseId', filterKey: 'courseId' },
        {
            ...
            label: 'Start Date',
            sortKey: 'startDate',
            render: (item) => renderDateLearning(item.startDate)
        },
        {
            key: 'endDate',
            label: 'End Date',
            sortKey: 'endDate',
            render: (item) => renderDateLearning(item.endDate)
        },
    ...
    CourseEnrollment: [
        ...
        { key: 'score', label: 'Score', sortKey: 'score' },
        {
            ...
            label: 'Passed',
            sortKey: 'passed',
            render: (item) => renderBooleanLearning(item.passed, 'Pass', 'Fail')
        },
        {
            key: 'dueDate',
            label: 'Due Date',
            sortKey: 'dueDate',
            render: (item) => renderDateLearning(item.dueDate)
        }
    ],
    ...
    EmployeeCertification: [
        ...
        },
        {
            ...
            label: 'Issue Date',
            sortKey: 'issueDate',
            render: (item) => renderDateLearning(item.issueDate)
        },
        {
            key: 'expirationDate',
            label: 'Expiration',
            sortKey: 'expirationDate',
            render: (item) => renderDateLearning(item.expirationDate)
        },
        {
            key: 'isVerified',
            label: 'Verified',
            sortKey: 'isVerified',
            render: (item) => renderBooleanLearning(item.isVerified)
        }
    ],
    ...
    EmployeeSkill: [
        ...
        { key: 'yearsOfExperience', label: 'Years Exp.', sortKey: 'yearsOfExperience' },
        {
            ...
            render: (item) => renderBooleanLearning(item.isPrimarySkill)
        },
        {
            key: 'lastUsedDate',
            label: 'Last Used',
            sortKey: 'lastUsedDate',
            render: (item) => renderDateLearning(item.lastUsedDate)
        }
    ],
    ...
    TrainingRecord: [
        ...
        },
        {
            ...
            render: (item) => renderDateLearning(item.completedDate)
        },
        {
            key: 'expirationDate',
            label: 'Expiration',
        ...
        },
        {
            ...
            label: 'Compliant',
            sortKey: 'isCompliant',
            render: (item) => renderBooleanLearning(item.isCompliant)
        }
    ]
};
...
if (typeof window !== 'undefined') {
    window.Learning = {
        ...
        render: {
            ...
            money: renderMoneyLearning,
            boolean: renderBooleanLearning,
            date: renderDateLearning,
