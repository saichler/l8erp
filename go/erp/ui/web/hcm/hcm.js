...
function renderStatus(status) {
    const statusMap = {
        1: { label: 'Active', class: 'hcm-status-active' },
        0: { label: 'Inactive', class: 'hcm-status-inactive' },
        2: { label: 'Pending', class: 'hcm-status-pending' },
        3: { label: 'Terminated', class: 'hcm-status-terminated' }
    };

    const config = statusMap[status] || { label: status, class: '' };
    return `<span class="hcm-status ${config.class}">${config.label}</span>`;
}
