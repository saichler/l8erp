...
function renderCompensationType(type) {
    return COMPENSATION_TYPE[type] || 'Unknown';
}

function renderMeritIncreaseStatus(status) {
    const statusClasses = {
        1: 'hcm-status-inactive',    // Draft
        2: 'hcm-status-pending',     // Submitted
        3: 'hcm-status-pending',     // Under Review
        4: 'hcm-status-active',      // Approved
        5: 'hcm-status-terminated',  // Rejected
        6: 'hcm-status-active'       // Processed
    };
    const label = MERIT_INCREASE_STATUS[status] || 'Unknown';
    const cssClass = statusClasses[status] || '';
    return `<span class="hcm-status ${cssClass}">${label}</span>`;
}

function renderMeritCycleStatus(status) {
    const statusClasses = {
        1: 'hcm-status-pending',     // Planning
        2: 'hcm-status-active',      // Open
        3: 'hcm-status-pending',     // Under Review
        4: 'hcm-status-active',      // Approved
        5: 'hcm-status-inactive'     // Closed
    };
    const label = MERIT_CYCLE_STATUS[status] || 'Unknown';
    const cssClass = statusClasses[status] || '';
    return `<span class="hcm-status ${cssClass}">${label}</span>`;
}

function renderBonusPlanType(type) {
    return BONUS_PLAN_TYPE[type] || 'Unknown';
}

function renderBonusFrequency(freq) {
    return BONUS_FREQUENCY[freq] || 'Unknown';
}

function renderBonusFundingType(type) {
    return BONUS_FUNDING_TYPE[type] || 'Unknown';
}

function renderBonusPaymentStatus(status) {
    const statusClasses = {
        1: 'hcm-status-inactive',    // Draft
        2: 'hcm-status-pending',     // Pending Approval
        3: 'hcm-status-active',      // Approved
        4: 'hcm-status-pending',     // Scheduled
        5: 'hcm-status-active',      // Paid
        6: 'hcm-status-terminated'   // Cancelled
    };
    const label = BONUS_PAYMENT_STATUS[status] || 'Unknown';
    const cssClass = statusClasses[status] || '';
    return `<span class="hcm-status ${cssClass}">${label}</span>`;
}
...
function renderEquityGrantType(type) {
    const typeDescriptions = {
        ...
        2: 'NSO (Non-Qualified Stock Option)',
        3: 'RSU (Restricted Stock Unit)',
        4: 'RSA (Restricted Stock Award)',
        5: 'ESPP (Employee Stock Purchase)',
        6: 'Phantom Stock',
        7: 'SAR (Stock Appreciation Rights)'
    };
    return EQUITY_GRANT_TYPE[type] || 'Unknown';
}

function renderEquityGrantStatus(status) {
    const statusClasses = {
        1: 'hcm-status-pending',     // Pending
        2: 'hcm-status-active',      // Active
        3: 'hcm-status-active',      // Fully Vested
        4: 'hcm-status-active',      // Exercised
        5: 'hcm-status-inactive',    // Expired
        6: 'hcm-status-terminated',  // Forfeited
        7: 'hcm-status-terminated'   // Cancelled
    };
    const label = EQUITY_GRANT_STATUS[status] || 'Unknown';
    const cssClass = statusClasses[status] || '';
    return `<span class="hcm-status ${cssClass}">${label}</span>`;
}

function renderVestingType(type) {
    return VESTING_TYPE[type] || 'Unknown';
}

function renderVestingFrequency(freq) {
    return VESTING_FREQUENCY[freq] || 'Unknown';
}
...
function renderMoneyComp(money) {
...
function renderCompaRatio(ratio) {
    ...
    let cssClass = '';
    if (ratio < 0.85) cssClass = 'hcm-status-terminated';
    else if (ratio < 0.95) cssClass = 'hcm-status-pending';
    else if (ratio <= 1.05) cssClass = 'hcm-status-active';
    else if (ratio <= 1.15) cssClass = 'hcm-status-pending';
    else cssClass = 'hcm-status-terminated';
    return `<span class="hcm-status ${cssClass}">${percentage}%</span>`;
}
...
function renderSalaryRange(item) {
    const min = renderMoneyComp(item.minimum);
    const max = renderMoneyComp(item.maximum);
    if (min === '-' && max === '-') return '-';
    return `${min} - ${max}`;
}
...
const COMPENSATION_COLUMNS = {
    SalaryGrade: [
        ...
        },
        {
            ...
            label: 'Midpoint',
            sortKey: 'midpoint',
            render: (item) => renderMoneyComp(item.midpoint)
        },
        {
            key: 'payFrequency',
            label: 'Frequency',
        ...
        },
        {
            ...
            label: 'Active',
            sortKey: 'isActive',
            render: (item) => renderBooleanComp(item.isActive)
        }
    ],
    ...
    SalaryStructure: [
        ...
        },
        {
            ...
            render: (item) => renderDateComp(item.effectiveDate)
        },
        {
            key: 'isActive',
            label: 'Active',
    ...
    EmployeeCompensation: [
        ...
        },
        {
            ...
            label: 'Base Salary',
            sortKey: 'baseSalary',
            render: (item) => renderMoneyComp(item.baseSalary)
        },
        {
            key: 'payFrequency',
            label: 'Frequency',
    ...
    MeritIncrease: [
        ...
        { key: 'meritCycleId', label: 'Cycle', sortKey: 'meritCycleId', filterKey: 'meritCycleId' },
        {
            ...
            render: (item) => renderMoneyComp(item.currentSalary)
        },
        {
            key: 'proposedIncrease',
            label: 'Increase',
            sortKey: 'proposedIncrease',
            render: (item) => renderMoneyComp(item.proposedIncrease)
        },
        {
    ...
    MeritCycle: [
        ...
        },
        {
            ...
            label: 'Budget',
            sortKey: 'totalBudget',
            render: (item) => renderMoneyComp(item.totalBudget)
        },
        {
    ...
    BonusPayment: [
        ...
        },
        {
            ...
            render: (item) => renderMoneyComp(item.actualAmount)
        },
        {
            key: 'status',
            label: 'Status',
        ...
        },
        {
            ...
            label: 'Payment Date',
            sortKey: 'paymentDate',
            render: (item) => renderDateComp(item.paymentDate)
        }
    ],
    ...
    EquityGrant: [
        ...
        },
        {
            ...
            label: 'Grant Price',
            sortKey: 'grantPrice',
            render: (item) => renderMoneyComp(item.grantPrice)
        },
        {
            key: 'status',
            label: 'Status',
        ...
        },
        {
            ...
            label: 'Grant Date',
            sortKey: 'grantDate',
            render: (item) => renderDateComp(item.grantDate)
        }
    ],
    ...
    CompensationStatement: [
        ...
        },
        {
            ...
            label: 'Bonus',
            sortKey: 'bonusActual',
            render: (item) => renderMoneyComp(item.bonusActual)
        },
        {
            key: 'equityValue',
            label: 'Equity',
            sortKey: 'equityValue',
            render: (item) => renderMoneyComp(item.equityValue)
        },
        {
            key: 'totalCashCompensation',
            label: 'Total Cash',
            sortKey: 'totalCashCompensation',
            render: (item) => renderMoneyComp(item.totalCashCompensation)
        },
        {
            key: 'totalCompensation',
            label: 'Total Comp',
            sortKey: 'totalCompensation',
            render: (item) => renderMoneyComp(item.totalCompensation)
        }
    ],
    ...
    MarketBenchmark: [
        ...
        { key: 'surveyYear', label: 'Year', sortKey: 'surveyYear', filterKey: 'surveyYear' },
        {
            ...
            label: '50th %ile',
            sortKey: 'market50th',
            render: (item) => renderMoneyComp(item.market50th)
        },
        {
            key: 'market75th',
            label: '75th %ile',
            sortKey: 'market75th',
            render: (item) => renderMoneyComp(item.market75th)
        },
        {
            key: 'internalAverage',
            label: 'Internal Avg',
            sortKey: 'internalAverage',
            render: (item) => renderMoneyComp(item.internalAverage)
        },
        {
            key: 'marketIndex',
            label: 'Market Index',
...
if (typeof window !== 'undefined') {
    window.Compensation = {
        ...
        render: {
            ...
            money: renderMoneyComp,
            boolean: renderBooleanComp,
            date: renderDateComp,
