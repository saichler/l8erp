...
func Activate(creds, dbname string, vnic ifs.IVNic) {
	...
	sla := ifs.NewServiceLevelAgreement(&persist.OrmService{}, ServiceName, ServiceArea, true, newEmployeeServiceCallback())
...
func Employees(vnic ifs.IVNic) (ifs.IServiceHandler, bool) {
	return vnic.Resources().Services().ServiceHandler(ServiceName, ServiceArea)
}

func Employee(employeeId string, vnic ifs.IVNic) (*hcm.Employee, error) {
	this, ok := Employees(vnic)
	if !ok {
		return nil, errors.New("No Employee Service Found")
	}
	filter := &hcm.Employee{EmployeeId: employeeId}
	resp := this.Get(object.New(nil, filter), vnic)
	if resp.Error() != nil {
		return nil, resp.Error()
	}
	return resp.Element().(*hcm.Employee), nil
}
