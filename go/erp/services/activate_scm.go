...
func ActivateSCMServices(creds, dbname string, nic ifs.IVNic) {
	...
	requisitionlines.Activate(creds, dbname, nic)
	rfqs.Activate(creds, dbname, nic)
	purchaseorders.Activate(creds, dbname, nic)
	polines.Activate(creds, dbname, nic)
	...
	stockmovements.Activate(creds, dbname, nic)
	lotnumbers.Activate(creds, dbname, nic)
	serialnumbers.Activate(creds, dbname, nic)
	cyclecounts.Activate(creds, dbname, nic)
	reorderpoints.Activate(creds, dbname, nic)
	inventoryvaluations.Activate(creds, dbname, nic)
	// Warehouse Management
	warehouses.Activate(creds, dbname, nic)
	bins.Activate(creds, dbname, nic)
	receivingorders.Activate(creds, dbname, nic)
	putawaytasks.Activate(creds, dbname, nic)
	picktasks.Activate(creds, dbname, nic)
	packtasks.Activate(creds, dbname, nic)
	shiptasks.Activate(creds, dbname, nic)
	...
	deliveryproofs.Activate(creds, dbname, nic)
	freightaudits.Activate(creds, dbname, nic)
	...
	forecastaccuracies.Activate(creds, dbname, nic)
	// Supply Planning
